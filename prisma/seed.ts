...
async function seedAdminUser() {
  const users = [
    ...
    { username: 'editor', password: 'Editor@123', role: 'Admin' },
  ]

  for (const u of users) {
    const exists = await prisma.adminUser.findFirst({ where: { username: u.username } })
    if (!exists) {
  ...
  console.log(`✓ admin_user: ${users.length} accounts (admin/Admin@123, editor/Editor@123)`)
}
...
async function main() {
  ...
  console.log('Order: system_setting → lottery_type → theme → layout → play_rule → attribute_library → admin_user')
  console.log()
